...
void ReceiverX::clearCan()
{
	...
	while(character == CAN)
	{
		int res = PE(myReadcond(mediumD, &character, 1, 1, TM_2CHAR, TM_2CHAR));

		if(res == 0)		//if it didn't read
		{
			break; 				//in case the timeout happens, &character might not be overwritten
		}
...
void ReceiverX::purge()
{
	...
	while(con)
	{
		int res = PE(myReadcond(mediumD, temp, BUF_SZ, 1, TM_CHAR, TM_CHAR));		//maybe restBlkSz

	//if it doesn't read
		if(res == -1)			// -1 = error; you should never reach this stage according to TA
		{
			COUT<<"error in receiverX::purge()"<< endl;
			break;
		}
		else if (res ==0)
		{
