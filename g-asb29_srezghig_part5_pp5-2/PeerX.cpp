//% Student Name 1: Siavash Rezghi
//% Student 1 #: 301311417
//% Student 1 userid (email): srezghig@sfu.ca
//
//% Student Name 2: Arshdeep Singh Bhullar
//% Student 2 #: 301326478
//% Student 2 userid (email): asb29@sfu.ca
//
//% Below, edit to list any people who helped you with the code in this file,
//%      or put 'None' if nobody helped (the two of) you.
//
// Helpers: All of the helps were from the TAs (Zavier and Mariyam)
//
...
transferCommon(std::shared_ptr<StateMgr> mySM, bool reportInfoParam)
{
	...
	while(mySM->isRunning()) {
		...
		if (now >= absoluteTimeout) {
			...
			tv.tv_usec = absoluteTimeout - now;	//how much time we have left
			int res = PE(select(max(consoleInId, mediumD)+1, &set, NULL, NULL,&tv));
			...
			if(res == -1){
				//do nothing
				COUT << "something wrong in trasnfercommon in peerX" << endl;
			}
			...
			else if (FD_ISSET(consoleInId, &set))
			{
				//check kb_canc
				string str;
				myReadcond(consoleInId, &str, 3, 0, TM_2CHAR, TM_2CHAR);
				str = str +'\0';
